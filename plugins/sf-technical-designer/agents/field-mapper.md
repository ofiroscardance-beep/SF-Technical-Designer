...
2. **Write** it to `output/fields.json` in the current project.
3. **Render** it by running (the plugin's SessionStart hook has prepared a venv
   ...
   `"$SFTD_PYTHON" -m src.fields_builder output/fields.json output/<slug>_fields.xlsx`
...
absolute paths. If `src.fields_builder` itself is unavailable, fall back to the
`xlsx` skill, preserving the same index sheet, per-entity tabs, two sections,
columns, colours, and RTL.
