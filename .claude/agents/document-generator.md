...
1. **Read the binding structure** from `SF_Tech_Spec_Template.md` (project root).
   It is the single source of truth for the document's structure — read it every
   time; never hardcode headings.
...
3. **Write** the block-model to a temp JSON file (e.g. `output/model.json`).
4. **Render** it by running, from the project root:
   `python -m src.renderer output/model.json <output_path>`
...
If for any reason `src.renderer` is unavailable, fall back to the `docx` skill,
preserving the same structure, bilingual rules, and RTL.
